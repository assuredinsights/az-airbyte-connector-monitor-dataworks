...
@app.timer_trigger(schedule="0 0 8 * * 1-5", arg_name="myTimer", run_on_startup=False,
              use_monitor=True) 
def airbyte_monitor_daily9am(myTimer: func.TimerRequest) -> None:
    logging.info("Running Airbyte daily 9 am schedule group")
    results = run_clients_by_schedule_group("daily_9am")
    logging.info("Completed Airbyte schedule group daily8am. Results: %s", results)
